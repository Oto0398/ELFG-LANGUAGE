#2023/2024 Project -Alternative Generator of ELFG Language- by Kassandra Briola, Samuele Antonelli, Otari Tchigladze
...
# --------------------- PART I Get translation from English files and create dictionary stored into Json file----------------------------------
...
#Function prompts the user to add a new sentence to the eng.txt file.
#If enter 1 then print new sentences 
# if enter 2 then run the code with the existing file
def add_sentence():
    while True:
        new_sentence = input("Do you want to add another sentence? (1 for yes, 2 for no): ")
        if new_sentence == "1":
            new_sentence = input("Enter the sentence to be added: ")
            with open("eng.txt", "a") as f:
                f.write("\n" + new_sentence)
            print("Sentence added successfully.")
        elif new_sentence == "2":
            print("Continuing with the existing text...")
            break
        else:
            print("Invalid input. Continuing with the existing text...")
